...
import { clearSizeCache, cacheFileName } from '../src/package-info';
...
function importCost(fileName: string, language: string | null = null, config = DEFAULT_CONFIG) {
    if (!language) {
...
async function test(fileName: string, pkg = 'chai', minSize = 10000, maxSize = 15000, gzipLowBound = 0.01, gzipHighBound = 0.8) {
    const packages = await getPackages(fileName);
    const size = sizeOf(packages, pkg);
    expect(size).to.be.within(minSize, maxSize);
    expect(gzipOf(packages, pkg)).to.be.within(size * gzipLowBound, size * gzipHighBound);
}
...
describe('importCost', () => {
    ...
    it('ignores order of javascript imports for caching purposes', async () => {
        ...
        expect(await timed(() => test('import-mixed-reversed.js'))).to.be.within(0, 120);
    });
    ...
    it('debounce any consecutive calculations of same import line', () => {
        const p1 = expect(whenDone(runner(fixture('import.js'), 'import "chai";', JAVASCRIPT, DEFAULT_CONFIG))).to.be.rejectedWith(DebounceError);
        const p2 = expect(whenDone(runner(fixture('import.js'), 'import "chai/index";', JAVASCRIPT, DEFAULT_CONFIG))).to.be.fulfilled;
    ...
    it('errors on broken javascript', () => {
        return expect(whenDone(importCost(fixture('incomplete.bad.js')))).to.be.rejected;
    });
    it('errors on broken typescript', () => {
        return expect(whenDone(importCost(fixture('incomplete.bad.ts')))).to.be.rejected;
    });
    it('errors on broken vue', () => {
        return expect(whenDone(importCost(fixture('incomplete.bad.vue')))).to.be.rejected;
    });
