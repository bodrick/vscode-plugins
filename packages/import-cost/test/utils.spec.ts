...
describe('utils', () => {
    describe('getPackageDirectory', () => {
        it('should handle simple directory structure', () => {
            ...
            expect(getPackageDirectory(pkg)).to.eql(path.resolve('test', 'fixtures', 'node_modules', 'chai'));
        });

        it('should handle a nested project structure', () => {
            const pkg = {
                fileName: path.resolve('test', 'fixtures', 'yarn-workspace', 'import-nested-project.js'),
                name: 'chai'
            };
            expect(getPackageDirectory(pkg)).to.eql(path.resolve('test', 'fixtures', 'node_modules', 'chai'));
        });
        ...
        it('should handle a nested project structure, with scoped package', () => {
            const pkg = {
                fileName: path.resolve('test', 'fixtures', 'yarn-workspace', 'import-with-scope.js'),
                name: '@angular/core'
            };
            expect(getPackageDirectory(pkg)).to.eql(path.resolve('test', 'fixtures', 'node_modules', '@angular', 'core'));
        });

        it('should handle a nested project structure, with scoped package and filename', () => {
            const pkg = {
                fileName: path.resolve('test', 'fixtures', 'yarn-workspace', 'import-with-scope-filename.js'),
    ...
    describe('getPackageModuleContainer', () => {
        it('should return the node_modules dir in the basic case', () => {
            ...
            expect(getPackageModuleContainer(pkg)).to.eql(path.resolve('test', 'fixtures', 'node_modules'));
        });
